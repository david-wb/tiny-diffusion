...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("precache.log"),
        ...
        try:
            ...
            if os.path.exists(cache_path):
                try:
                    ...
                    logger.error(f"Corrupted cache file {cache_path}, redownloading: {e}")
...
def check_disk_space(path, required_space):
    ...
    if free_gb < required_gb:
        logger.error(f"Insufficient disk space: {free_gb:.2f} GB available, {required_gb:.2f} GB required")
        raise RuntimeError(f"Need at least {required_gb:.2f} GB free disk space")
    logger.info(f"Disk space check: {free_gb:.2f} GB available, {required_gb:.2f} GB required")
...
def precache_dataset(dataset, max_samples=None):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(cache_sample, idx): idx for idx in range(max_samples)}
        for future in tqdm(as_completed(futures), total=max_samples, desc="Pre-caching"):
...
def get_text_embeds(tokenizer, text_model, device, texts):
    ...
    with torch.no_grad():
        ...
        text_embeds = outputs.last_hidden_state[:, -1, :].to(device)  # Ensure embeds stay on device
    return text_embeds
...
class ForwardProcess:
    ...
    def q_sample(self, device, x_0, t):
        ...
        sqrt_one_minus_alphas_cumprod_t = torch.sqrt(1.0 - self.alphas_cumprod.to(device)[t]).view(-1, 1, 1, 1)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with autocast('cuda'):  # Updated API
                ...
                x_0 = latents
                t = torch.randint(0, forward_process.num_timesteps, (batch_size_actual,), device=device)
                x_t, noise = forward_process.q_sample(device, x_0, t)
                # Log device of tensors for debugging
                logger.debug(f"x_t device: {x_t.device}, t device: {t.device}, text_embeds device: {text_embeds.device}")
            ...
            if batch_idx % 10 == 0:
                wandb.log({"epoch": epoch, "batch": batch_idx, "batch_train_loss": loss})
            if batch_idx % 100 == 0:
                with torch.no_grad():
                    samples = sample(
                        noise_model,
                    ...
                    wandb_images = list(samples_to_wandb_images(samples, prompts_for_sample))
                    wandb.log({"epoch": epoch, "batch": batch_idx, "sampled_images": wandb_images})
        ...
        logger.info(f"Epoch {epoch}, Avg Train Loss: {avg_train_loss:.4f}, Avg Val Loss: {avg_val_loss:.4f}")
        ...
        wandb.log({"epoch": epoch, "samples": wandb.Image(fig, caption=f"Generated samples at epoch {epoch}")})
        plt.close(fig)
        noise_model.train()
    wandb.finish()
    ...
    for i in range(n_samples):
        ...
        if prompts is not None:
            axes[row][col].set_title(
                prompts[i][:15] + "..." if len(prompts[i]) > 15 else prompts[i], fontsize=9
            )
